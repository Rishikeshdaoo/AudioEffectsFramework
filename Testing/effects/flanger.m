function [y] = flanger(x, Fs, MaxDelayinSecs, ModRateinHz)
...
%
%The structure is:  [y] = flanger(audio_tset, x,Fs, MaxDelayinSecs, ModRateinHz)
%
%where audio_test = filtered audio from C++ code 
%      x = the input signal 
%      Fs = Sampling rate 
%      MaxDelayinSecs = delay time in seconds
%      ModRateinHz = rate of flange in Hz
%
...
sin_ref = (sin(2*pi*index*(ModRateinHz/Fs)))';
%convert delay in ms to max delay in samples
MaxDelayinSamps=round(MaxDelayinSecs*Fs);
